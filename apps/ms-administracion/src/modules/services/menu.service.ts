...
export class MenuService {
  ...
  async update(params:  PayloadData<MenuDTO>): Promise<MenuDTO> {
    ...
    const entityToUpdate = await this.menuManager.findOneBy(idUpdate);
    const result = await this.menuManager.updateBasic(dataUpdate,false,entityToUpdate);
    if(this.isAudit){
      await this.auditLogManager.logEvent('Edición','Menú',params.dataUser.id,dataUpdate);
    }
    return plainToInstance(MenuDTO, result);
  }

  async delete(params:  PayloadData<MenuDTO>): Promise<MutatioResult> {
    let  data = plainToInstance(MenuEntity, params.data) 
    data['usuariomodificacion_id'] = params.dataUser.id;
    data['estado'] = false;
    const dataUpdate = deleteNullArray(data);
