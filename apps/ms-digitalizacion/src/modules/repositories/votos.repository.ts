...
export class VotosRepository extends RepositoryOrmBase<VotosEntity> {
  ...
  /**
   * Función que actualiza los votos durante el proceso de control de calidad
   *
   * @async
   * @param {number} acta_id
   * @param {number} candidato_id
   ...
   * @returns {*}
   */
  async updateVotosControl(acta_id:number,candidato_id:number,votoscontrol:number,usuarioId:number,dataCifrado:string,queryRunner:any) {
    await queryRunner.query(
      `CALL actualizar_votos_control(${acta_id},${candidato_id},${votoscontrol},
      '${dataCifrado}','${generateSHA256Hash(dataCifrado)}',${usuarioId});`
    ); 
  }
}
